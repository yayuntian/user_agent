...
vector<UATest> uastrings = {
        ...
        },
        {
                ...
                expected:   "Mozilla:5.0 Platform:Macintosh OS:Intel Mac OS X 10_6_5 Localization:en-US Browser"
                        ":Chrome-8.0.552.231 Engine:AppleWebKit-534.10 Bot:false Mobile:false"
        },
        {
                title:    "SafariMac",
        ...
        },
        {
                ...
                expected: "Mozilla:5.0 Platform:iPad OS:CPU OS 3_2 like Mac OS X Localization:en-us Browser:Safari-4.0.4 "
                        "Engine:AppleWebKit-531.21.10 Bot:false Mobile:true"
        },
        {
                title:    "webOS",
        ...
        },
        {
                ...
                expected:   "Mozilla:5.0 Platform:BlackBerry OS:BlackBerry 9800 Localization:en Browser:BlackBerry-6.0.0.141 "
                        "Engine:AppleWebKit-534.1+ Bot:false Mobile:true"
        },
        {
                title:    "BB10",
...
string format(UserAgent& ua) {
    ...
    if (!ua.localization.empty()) {
        s += "Localization:" + ua.localization + " ";
    }

    if (!ua.browser.Name.empty()) {
        s += "Browser:" + ua.browser.Name;
        if (!ua.browser.Version.empty()) {
            s += "-" + ua.browser.Version + " ";
        } else {
            s += " ";
        }
    }

    if (!ua.browser.Engine.empty()) {
        s += "Engine:" + ua.browser.Engine;
        if (!ua.browser.EngineVersion.empty()) {
            s += "-" + ua.browser.EngineVersion + " ";
        } else {
            s += " ";
        }
    }
...
void TestUserAgent() {
    ...
    for (auto p : uastrings) {
        ...
        if (p.expected != format(ua)) {
            ...
            p.title.c_str(), format(ua).c_str(), p.expected.c_str());
            exit(1);
        }
    }
